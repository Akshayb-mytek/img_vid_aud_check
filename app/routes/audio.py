...
from app.services.audio.workers.pool import run_in_pool
from app.services.audio.pipeline import run_audio_verification
from app.services.audio.utils.errors import AudioVerificationError
from app.schemas_audio.responses import VerificationResult, BatchVerificationResult
from app.services.audio.constants import ReasonCode, DIARIZATION_DISTANCE_THRESHOLD
import structlog
import asyncio
import numpy as np
...
async def check_audio(
    ...
    try:
        ...
        from app.config import settings
        raise HTTPException(status_code=408, detail={"code": "REQUEST_TIMEOUT", "message": f"Verification timed out after {settings.audio_request_timeout_seconds} seconds."})
