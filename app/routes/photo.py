...
from app.schemas_photo.responses import VerificationResult, BatchResults, IdentityConsistency, PrimaryReason
from app.services.photo.pipeline import verify_image
from app.services.photo.identity_checks import verify_identity_consistency, get_face_embedding, compute_cosine_distance
from app.services.photo.workers.pool import get_pool
...
async def check_single(
    ...
    try:
        ...
        result: VerificationResult = await asyncio.wait_for(
            ...
            timeout=settings.photo_batch_timeout_seconds
        )
...
async def check_batch(
    ...
    if len(valid_images_for_identity) > 1:
        try:
            ...
            overall_passed = False
            from app.schemas_photo.responses import PrimaryReason
